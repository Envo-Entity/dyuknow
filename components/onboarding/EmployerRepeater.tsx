    ...
    <div className="flex flex-col gap-3">
      ...
      {value.map((entry, index) => (
        <div key={index} className="flex flex-col gap-3 rounded-2xl border border-border-soft bg-mist p-4">
          <div className="flex items-center justify-between">
            <span className="text-[12.5px] font-semibold text-muted">Employer {index + 1}</span>
            <button
              type="button"
              onClick={() => remove(index)}
              className="cursor-pointer text-[12.5px] font-semibold text-ink underline underline-offset-2"
            >
              Remove
            </button>
          </div>
      ...
      {value.length < MAX_EMPLOYERS && (
        <button
          ...
          className="flex cursor-pointer items-center justify-center gap-2 rounded-2xl border border-dashed border-hairline py-3 text-[13px] font-semibold text-ink transition-colors hover:border-ink"
        >
          <PlusIcon size={13} />
